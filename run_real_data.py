...
# 加载真实数据
print("\n正在加载真实收益率数据...")
returns_df = pd.read_csv('/workspace/returns_20260511_153215.csv', index_col=0, parse_dates=True)
print(f"数据加载成功！")
...
csn_tc = float(summary.loc['CSN-HMC', '年化换手率(%)'])
mv_tc = float(summary.loc['MV', '年化换手率(%)'])
...
ax1.set_title('累计净值曲线 (扣除交易成本后)', fontsize=16, fontweight='bold', pad=20)
...
plt.tight_layout()
output_path = '/workspace/backtest_final_results.png'
...
w_csn.to_csv(f'/workspace/csn_hmc_weights_{timestamp}.csv', encoding='utf-8-sig')
print(f"\n结果已保存到 CSV 文件（带时间戳）")
